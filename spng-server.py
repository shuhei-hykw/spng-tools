  ...
  return render_template_string("""
    ...
    <head>
      ...
      <style>
        ...
        .btn-proj { background: #225522; border-color: #448844; }
    ...
    <body>
      <div id="sidebar">
        <div class="sidebar-section">
          ...
          {% if selected_json %}
          <!-- <button class="btn btn-proj" onclick="showProjection()">SHOW PROJECTION (SUP)</button> -->
          {% endif %}
        </div>
      ...
      <div id="main">
        ...
        <script>
          ...
          function showProjection() {
            idxDisp.innerText = "PROJECTION";
            targetImg.src = '/projection/' + relPath;
          }
          ...
          viewport.addEventListener('mousedown', (e) => {
            isDown = true; startX = e.pageX - viewport.offsetLeft; startY = e.pageY - viewport.offsetTop;
...
@app.route('/projection/<path:json_rel_path>')
def get_projection(json_rel_path):
  json_path = (ROOT_DIR / json_rel_path).resolve()
  try:
    with json_path.open("r", encoding="utf-8") as f:
      meta = json.load(f)
    ...
    # Simple Summation and Normalization
    summed = None
    for entry in images:
      parts = entry["Path"].split("&")
      spng_file = json_path.parent / parts[0]
      raw_data = read_png_from_spng(spng_file, int(parts[1]), int(parts[2]))
      ...
      if summed is None:
        ...
        summed = cv2.add(summed, img_float)

    if summed is not None:
      # Normalization: Map the range [min, max] to [0, 255]
      # This ensures the brightest pixel becomes 255 and darkest becomes 0
      norm_img = cv2.normalize(summed, None, 0, 255, cv2.NORM_MINMAX, cv2.CV_8U)
    else:
      norm_img = np.zeros((1, 1), dtype=np.uint8)
